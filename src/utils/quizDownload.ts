...
export const downloadAnswerSheetPDF = (options: DownloadOptions) => {
  const { eventTitle, topicName, quizzes, results, candidateName = 'Player', language = 'en' } = options;
  const doc = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: 'a4'
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(16);
  doc.text('OFFICIAL ANSWER KEY & REPORT CARD', margin + 6, y + 11);

  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(148, 163, 184); // Slate-400
  doc.text(`Event Name: ${eventTitle}`, margin + 6, y + 17);
  doc.text(`Topic Name: ${topicName}`, margin + 6, y + 22);
  doc.text(`Candidate Name: ${candidateName.toUpperCase()}`, margin + 6, y + 27);

  // Stats badge on the right of banner
  doc.setFillColor(30, 41, 59); // background for stats block
  doc.rect(pageWidth - margin - 55, y + 4, 49, 26, 'F');
  doc.setDrawColor(50, 190, 250);
  doc.setLineWidth(0.5);
  doc.rect(pageWidth - margin - 55, y + 4, 49, 26, 'D');
  ...
  doc.text(`Date: ${results ? new Date(results.completedAt).toLocaleDateString() : 'N/A'}`, pageWidth - margin - 30, y + 26, { align: 'center' });
  ...
  doc.setFontSize(11);
  doc.setTextColor(15, 23, 42);
  doc.text('QUESTION REVIEW & RESPONSE MATRIX', margin, y);
  ...
  quizzes.forEach((quiz, index) => {
    ...
    optionsToShow.forEach((opt, optIdx) => {
      ...
      // Draw beautiful OMR bubble circle
      ...
      if (isChosen) {
        if (isCorrect) {
          // Chosen correct: Emerald Solid
          doc.setFillColor(16, 185, 129);
          doc.setDrawColor(16, 185, 129);
          doc.circle(circleX, circleY, r, 'F');
          doc.setTextColor(255, 255, 255);
        } else {
          // Chosen wrong: Red Solid
        ...
        // Regular option (unattempted/unchosen): empty slate bubble
      ...
      // Draw option identifier (A, B, C, D) inside the center of the bubble
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(7.5);
  ...
  // Save the PDF
  const filename = `Answer_Sheet_${candidateName.replace(/\s+/g, '_')}_${eventTitle.replace(/\s+/g, '_')}.pdf`;
  doc.save(filename);
};
