...
export default function AdminPanel() {
  ...
  const syncAllPublicProfiles = async () => {
    const verified = await confirm({
      title: 'Sync All Profiles',
      description: 'This will recreate public profiles for all approved users to ensure they show up on the leaderboard. Continue?',
      type: 'confirm'
    });
    if (!verified) return;
    ...
    try {
      ...
      let syncCount = 0;
      for (const [uid, u] of Object.entries(allUsersData) as [string, any][]) {
        if (u.status === 'approved') {
          await update(ref(db, `public_profiles/${uid}`), {
            id: uid,
            name: u.name,
      ...
      await alert({ title: 'Success', description: `Synced ${syncCount} profiles!`, type: 'success' });
    } catch (e: any) {
      console.error(e);
      await alert({ title: 'Sync Failed', description: e.message, type: 'error' });
    }
  };
  ...
  const createPlayerAccount = async () => {
    ...
    try {
      ...
      // Create public profile for discoverability
      await set(ref(db, `public_profiles/${uid}`), {
        id: uid,
        name: newUser.name,
               ...
               <div className="space-y-4">
                  <div className="flex items-center justify-between px-2">
                     <div className="flex items-center gap-2">
                        ...
                        <button 
                          onClick={syncAllPublicProfiles}
                          className="p-2 bg-primary/10 text-primary rounded-xl hover:bg-primary hover:text-black transition-all group"
                          title="Sync All Public Profiles"
                        >
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {quizzes.slice().reverse().map((q, qIdx) => (
                   <div key={`quiz-${q.id}-${qIdx}`} className="bg-black/5 dark:bg-black/60 border border-black/5 dark:border-white/5 p-5 rounded-[2rem] group relative overflow-hidden">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {q.options?.en?.map((opt, i) => (
                           <div key={`quiz-${q.id}-opt-${i}-${qIdx}`} className={cn(
